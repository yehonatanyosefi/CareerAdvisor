...
const getJSONData = async (folder, filename) => {
  const filePath = path.join(process.cwd(), "data", folder, `${filename}.json`);
  const fileData = fs.readFileSync(filePath, "utf8");
  return JSON.parse(fileData);
};

const fetchDataFromFiles = async (username) => {
  try {
    const fileNames = ['Demographic','Description','Personality'];
    const data = await Promise.all(fileNames.map(filename => getJSONData(username, filename)));
    return data;
  } catch (err) {
    console.error(err);
    throw err;
...
const initializeChain = async (initialPrompt, transcript) => {
  try {
      userData = await fetchDataFromFiles(username);
...
export default async function handler(req, res) {
  if (req.method === "POST") {
    ...
    const { prompt, firstMsg, username } = req.body;
